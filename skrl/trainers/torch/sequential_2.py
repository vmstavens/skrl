...
def rollout(env, agent, timestep) -> None:
    num_timesteps = 100
    state, _ = env.reset()
    frames = []
    # print(env)
    ...
    # print(env.unwrapped.unwrapped._env.unwrapped)
    # env = env.unwrapped.unwrapped._env.unwrapped
    # quit()
    video_filename = f"media/test_{timestep}.mp4"
...
class SequentialTrainer2(Trainer):
    def __init__(
        self,
        env: Wrapper,
        eval_env: Wrapper,
        ...
        """
        _cfg = copy.deepcopy(SEQUENTIAL_TRAINER_2_DEFAULT_CONFIG)
    ...
    def single_agent_train(self) -> None:
        ...
        for timestep in tqdm.tqdm(
            ...
            # if timestep % self.cfg["eval_every_n_steps"] == 0:
            #     self.agents.set_mode("eval")
            #     rollout(self.eval_env, self.agents, timestep)
