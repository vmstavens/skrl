...
from utils.mjw import get_pose


def domain_randomize(model: mjw.Model, mj_model: mj.MjModel, rng: jax.Array):
    mj_model = mj_model
...
class PipeInsert2(mjx_env.MjxEnv):
    ...
    def __init__(
        ...
        self._mjx_model: mjw.Model = mjw.put_model(self._mj_model)
    ...
    def _site_translation(
        self, data: mjw.Data, site_id: int, *, world_id: int = 0
    ) -> jax.Array:
        pose = get_pose(data, [int(site_id)], world_id=world_id)
        pose_np = pose.numpy()
        return jp.array(pose_np[0, :3])

    def _get_distance(self, data: mjw.Data) -> jax.Array:

        target_1 = self._site_translation(data, self.target_ids[0])
        target_2 = self._site_translation(data, self.target_ids[1])
        target_3 = self._site_translation(data, self.target_ids[2])

        keypoint_1 = self._site_translation(data, self.keypoint_ids[0])
        keypoint_2 = self._site_translation(data, self.keypoint_ids[1])
        keypoint_3 = self._site_translation(data, self.keypoint_ids[2])

        dist_to_target_1 = jp.linalg.norm(target_1 - keypoint_1)
        dist_to_target_2 = jp.linalg.norm(target_2 - keypoint_2)
        dist_to_target_3 = jp.linalg.norm(target_3 - keypoint_3)
    ...
    def _get_done(self, data: mjw.Data) -> float:

        # 1) get the positions of the keypoints
        # 2) get the points at the entry and exit of the pipe
        #
        pipe_entry = self._site_translation(data, self.pipe_entry_id)
        pipe_exit = self._site_translation(data, self.pipe_exit_id)
    ...
    def _get_reward(self, data: mjw.Data) -> float:
        return -self._get_distance(data)

    def _get_site_pos(self, data: mjw.Data) -> jax.Array:
    ...
    def _from_to(self, data: mjw.Data, _from: int, _to: int) -> jax.Array:
        p_from = self._site_translation(data, _from)
        p_to = self._site_translation(data, _to)
        return p_to - p_from

    def _get_obs(self, data: mjw.Data, info: dict[str, Any]) -> jax.Array:
    ...
    def observation_size(self) -> int:
        ...
        # dummy_data = mjw.make_data(self.mjx_model)
        d = mj.MjData(self.mj_model)
        dummy_data = mjw.put_data(self.mj_model, d)
        dummy_info = {}
