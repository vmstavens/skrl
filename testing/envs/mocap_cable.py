...
def _build_model() -> mj.MjModel:
    ...
    # 2) mocap
    mocap_pos = [-0.1, -0.1, 0.5]
    mocap_euler = [3.14, 0, 0]
    b_mocap = scene.worldbody.add_body(
        name="mocap", mocap=True, pos=mocap_pos, euler=mocap_euler
    )
    b_mocap.add_geom(
        name="mocap",
        type=mj.mjtGeom.mjGEOM_BOX,
    ...
    #     name="gripper", pos=mocap_pos, euler=mocap_euler
    # )
    ...
    cable = mjx_cable(count=cable_count, size=cable_size)
    cable.body("cable:B5").add_site(name="cable_seg5", pos=[0.0, 0.0, 0.0])
    cable.body("cable:Bfirst").add_site(name="cable_tip", pos=[0.0, 0.0, 0.0])
...
class MocapReach(mjx_env.MjxEnv):
    def __init__(self, config: config_dict.ConfigDict = default_config()):
        ...
        self._ee_site_id = _as_index(self._mj_model.site("mocap").id)
        self._goal_site_id = _as_index(self._mj_model.site("goal").id)
        self._cable_tip_site_id = _as_index(self._mj_model.site("cable_tip").id)
        ...
        self._workspace_low = self._goal_low
        self._workspace_high = self._goal_high
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        tip_err = jp.linalg.norm(obs[:3])
        grip_err = jp.linalg.norm(obs[3:6])
        reward = -(tip_err + self._grip_weight * grip_err)
        done = tip_err < self._success_threshold_item
    ...
    def _get_obs(self, data: mjx.Data) -> jax.Array:
        ...
        tip_err = goal_pos - tip_pos
        grip_err = tip_pos - ee_pos
        if data.ctrl.shape[0] > 0:
            ...
            ctrl_norm = jp.array(0.0)
        return jp.concatenate([tip_err, grip_err, jp.array([ctrl_norm])])
...
def main() -> None:
    ...
    final_tip_err = np.linalg.norm(final_obs[:3])
    final_grip_err = np.linalg.norm(final_obs[3:6])
    print(
        "final errors (unbatched): "
        f"tip={float(final_tip_err):.4f}, grip={float(final_grip_err):.4f}"
    )
