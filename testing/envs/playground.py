...
if _CPU:
    logger.warning(
        "Isaac Lab runs on GPU, but there is no GPU backend for JAX. JAX operations will run on CPU."
    )
...
def _torch2jax(tensor):
    if version.parse(jax.__version__) >= version.parse("0.7.0"):
        return jax_dlpack.from_dlpack(
            tensor.contiguous().cpu() if _CPU else tensor.contiguous()
        )
    return jax_dlpack.from_dlpack(
...
class PlaygroundWrapper(Wrapper):
    def __init__(self, env: Any) -> None:
        ...
        if isinstance(observation_size, dict):
            self._observation_space = gymnasium.spaces.Box(
                low=-np.inf, high=np.inf, shape=observation_size["state"]
            )
            ...
            self._observation_space = gymnasium.spaces.Box(
                low=-np.inf, high=np.inf, shape=(observation_size,)
            )
        ...
        print(f"{config.jax.key=}")
        print(f"{self.num_envs=}")
        self._env_reset_key = jax.random.split(config.jax.key, self.num_envs)
        print(f"{self._env_reset_key=}")
    ...
    def step(
        ...
        if self._state_space is None:
            self._observations = flatten_tensorized_space(
                tensorize_space(
                    self.observation_space, _jax2torch(self._env_state.obs, self.device)
                )
            )
        else:
            self._observations = flatten_tensorized_space(
                tensorize_space(
                    ...
                    _jax2torch(self._env_state.obs["state"], self.device),
                )
            )
            self._states = flatten_tensorized_space(
                tensorize_space(
                    self.state_space,
                    _jax2torch(self._env_state.obs["privileged_state"], self.device),
                )
            )
        ...
        if self._reset_once:
            ...
            if self._state_space is None:
                self._observations = flatten_tensorized_space(
                    tensorize_space(
                        self.observation_space,
                        _jax2torch(self._env_state.obs, self.device),
                    )
                )
            else:
                self._observations = flatten_tensorized_space(
                ...
                self._states = flatten_tensorized_space(
                    tensorize_space(
                        self.state_space,
                        _jax2torch(
                            self._env_state.obs["privileged_state"], self.device
                        ),
                    )
                )
    ...
    def render(self, *args, **kwargs) -> None:
        ...
        if self.num_envs > 1:
            logger.warning(
                "Rendering is not supported for parallel environments. Rendering will be skipped"
            )
            return
        ...
        frame = self._unwrapped.render(
            self._env_state, width=640, height=480, scene_option=scene_option
        )
