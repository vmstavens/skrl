...
class DRLR(Agent):
    def __init__(
        ...
        # self.IL_policy.set_running_mode("eval")
        # self.IL_policy.set_mode("eval")
        ...
        self._action_trans_high: list[float] = cfg["action_trans_high"]
        self._action_trans_low: list[float] = cfg["action_trans_low"]
        self._action_rot_high: list[float] = cfg["action_rot_high"]
        self._action_rot_low: list[float] = cfg["action_rot_low"]

        self._a_min_lim: list[float] = cfg["a_min_lim"]
        self._a_max_lim: list[float] = cfg["a_max_lim"]
    ...
    def _unnormalize_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        # action is assumed to be in [-1, 1]
        a_trans = action[:, :3]
        a_rot = action[:, 3:]

        trans_low = self._action_trans_low
        trans_high = self._action_trans_high
        rot_low = self._action_rot_low
        rot_high = self._action_rot_high

        # Map from [-1, 1] -> [low, high]
        a_trans = 0.5 * (a_trans + 1.0) * (trans_high - trans_low) + trans_low
        a_rot = 0.5 * (a_rot + 1.0) * (rot_high - rot_low) + rot_low

        unnormalized_action = torch.cat([a_trans, a_rot], dim=1)

        # print(f"{unnormalized_action.shape=}")
    ...
    def _normalize_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        trans_low = torch.as_tensor(
            self._action_trans_low, dtype=action.dtype, device=action.device
        )
        trans_high = torch.as_tensor(
            self._action_trans_high, dtype=action.dtype, device=action.device
        )
        rot_low = torch.as_tensor(
            self._action_rot_low, dtype=action.dtype, device=action.device
        )
        rot_high = torch.as_tensor(
            self._action_rot_high, dtype=action.dtype, device=action.device
        )

        trans_scale = torch.where(
            trans_high > trans_low,
            trans_high - trans_low,
            torch.ones_like(trans_high),
        )
        rot_scale = torch.where(
            rot_high > rot_low,
            rot_high - rot_low,
            torch.ones_like(rot_high),
        )

        # Map from [low, high] -> [-1, 1]
        a_trans = 2.0 * (a_trans - trans_low) / trans_scale - 1.0
        a_rot = 2.0 * (a_rot - rot_low) / rot_scale - 1.0


        normalized_action = torch.cat([a_trans, a_rot], dim=1).clamp(-1.0, 1.0)
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.autocast(
                ...
                with torch.no_grad():
                    ...
                    # print(f"{sampled_rewards.mean()=}")
                    # print(f"{entropy_target.mean()=}")
    ...
    def set_mode(self, mode: str) -> None:
        ...
        # self._warmup_timesteps = 0

        # print(f"in set_mode setting to '{mode}' for il modules")
        for model in self.models_il.values():
            if model is not None:
                model.set_mode(mode)
            else:
                print("obs a il model is None!!!")

        # print(f"in set_mode setting to '{mode}' for rl modules")
