...
class IBRL(Agent):
    ...
    def _select_act(
        ...
        if self._decision_block:
            ...
            if torch.mean(target_q_il) > torch.mean(target_q_rl):
                ...
                actions = rl_actions
                action_sources = torch.zeros(
                    (batch_size, 1), device=rl_actions.device, dtype=torch.long
                )
        else:
            # Stack Q-values
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        # actions, noises = self._apply_exploration_noise(actions, timestep, timesteps)
