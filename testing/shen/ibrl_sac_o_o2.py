...
class IBRL(Agent):
        ...
        if self.memory is not None:
            ...
            if not self._offline:
                ...
                if samples_to_add > 0:
                    ...
                    # Add samples in chunks if needed
                    start_idx = 0
                    while start_idx < samples_to_add:
                        ...
                        for tn in self._tensors_names:
                            tensor = self.expert_memory.get_tensor_by_name(tn).squeeze(
                                1
                            )[start_idx:end_idx, :]
    ...
    def _select_act(
        ...
        rl_actions = rl_actions.squeeze(0)
        ...
        if self._decision_block:
            ...
            if torch.mean(target_q_il) > torch.mean(target_q_rl):
                il_actions, _, _ = self.IL_policy.act(
                    {"states": self._state_preprocessor(il_obs)},
                    # {"states": self._state_preprocessor(obs)},
                    role="policy",
                    # {"states": self._state_preprocessor(obs)}, role="policy"
                )
                # TODO: convert action format into non-trajectory
                actions = il_actions
    ...
    def post_interaction(
        self, terminated: torch.BoolTensor, timestep: int, timesteps: int
    ) -> None:
