...
IBRL_SAC_DEFAULT_CONFIG = {
    ...
    # added
    "soft_update_beta": 10,         # use small for good IL (small ~0.2)
...
class IBRL(Agent):
    ...
    def _select_act(
        ...
        if self._decision_block:
            ...
            # il_obs has the structure [prev_obs, ]
            il_obs = il_obs[:, 0, :]
            # OSB ---------------------------------------------------
            # Does his need to be 0 or -1?
            # OSB ---------------------------------------------------
        ...
        if self._decision_block:
            ...
            if soft:
                ...
                if self._actor == "rl":
                    self.track_data("Which / il_selection_ratio", 0.0)

                elif self._actor == "il":
                    self.track_data("Which / il_selection_ratio", 1.0)

                elif (self._actor == "both") and (timestep < self._warmup_timesteps):
                    self.track_data("Which / il_selection_ratio", 1.0)

                elif (self._actor == "both") and (timestep >= self._warmup_timesteps):
                    self.track_data("Which / il_selection_ratio", pct_of_il)
                else:
                    raise ValueError(
                        f"[ERROR]: {self._actor=}, has to be ('rl', 'il' or 'both')"
                    )

                actions = rl_il_actions[
