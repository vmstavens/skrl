...
class DRLR(Agent):
    ...
    def _unnormalize_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        num_trans = min(3, action_dim)
        num_rot = max(0, action_dim - num_trans)

        low = torch.tensor(
            [self._action_trans_low] * num_trans + [self._action_rot_low] * num_rot,
            device=action.device,
            dtype=action.dtype,
        )
        high = torch.tensor(
            [self._action_trans_high] * num_trans + [self._action_rot_high] * num_rot,
            device=action.device,
            dtype=action.dtype,
        )
