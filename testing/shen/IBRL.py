...
from testing.shen.IBRLbase import Agent
...
class IBRL(Agent):
    ...
    def _select_act(self, obs: torch.Tensor, soft: bool, target: bool):
        if target:
            ...
            if self._backward_compatibility:
                rl_actions = torch.max(
                    torch.min(rl_actions, self.clip_actions_max), self.clip_actions_min
                )
            else:
    ...
    def _update(
        ...
        for gradient_step in range(self._gradient_steps):
                ...
                with torch.no_grad():
                    if self._offline:
                        ...
                        next_actions, _, _ = self._select_act(
                            sampled_next_states, soft=False, target=True
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                avg_critic_values = torch.stack(policy_critic_values_list, dim=0).mean(
                    dim=0
                )
