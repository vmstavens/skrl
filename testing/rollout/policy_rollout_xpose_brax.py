...
# from testing.envs.env_brax import XPose
...
def setup_environment(env_name: str = "xpose", num_envs: int = 256, debug=False):
    ...
    # envs.register_environment("xpose", XPose)
    # env = envs.create("xpose",batch_size=num_envs)
    env = mjx_XPose()
    ...
    env = brax_VmapWrapper(env, batch_size=num_envs)
    env = wrap_for_brax_training(env)

    envs.register_environment("xpose", mjx_XPose)

    env = envs.create("xpose", batch_size=4092)
...
env = setup_environment("xpose", num_envs=1, debug=True)
...
model_path = "testing/results/models/train_brax/checkpoints/best_agent.pt"
...
# Video setup
frames = []
video_filename = "policy_rollout_xpose_brax.mp4"
