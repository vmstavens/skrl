...
from testing.envs.cartpole import Balance
...
def setup_environment(num_envs: int = 256):
    """Set up the MJX cartpole environment with proper wrapping."""

    # Create the base environment
    env = Balance()

    # Apply vmap wrapper for parallel environments
    env = VmapWrapper(env, num_envs)

    # Use skrl's auto-wrapping
    env = wrap_env(env)
...
def main():
    ...
    env = setup_environment(num_envs=512)
    device = env.device
