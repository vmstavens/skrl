...
class Agent:
    def __init__(
        ...
        # if type(expert_memory) is list:
        #     self.expert_memory = expert_memory[0]
        # else:
        ...
        self.checkpoint_interval = self.cfg.get("experiment", {}).get(
            "checkpoint_interval", 1000
        )
        self.checkpoint_store_separately = self.cfg.get("experiment", {}).get(
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.cfg.get("experiment", {}).get("wandb", False):
            ...
            # set default values
            wandb_kwargs = copy.deepcopy(
                self.cfg.get("experiment", {}).get("wandb_kwargs", {})
            )
    ...
    def write_checkpoint(self, timestep: int, timesteps: int) -> None:
        ...
        if self.checkpoint_store_separately:
            for name, module in self.checkpoint_modules.items():
                torch.save(
                    ...
                    os.path.join(
                        self.experiment_dir, "checkpoints", "{}_{}.pt".format(name, tag)
                    ),
                )
        # whole agent
        else:
            ...
            torch.save(
                modules,
                os.path.join(
                    self.experiment_dir, "checkpoints", "{}_{}.pt".format("agent", tag)
                ),
            )
        ...
        #     self.checkpoint_best_modules["saved"],
        # )
                ...
                for name, module in self.checkpoint_modules.items():
                    torch.save(
                        ...
                        os.path.join(
                            self.experiment_dir,
                            "checkpoints",
                            "best_{}.pt".format(name),
                        ),
                    )
            # whole agent
            else:
                # print("\twriting models whole best →→→→→")
                modules = {}
                for name, module in self.checkpoint_modules.items():
                ...
                torch.save(
                    modules,
                    os.path.join(
                        self.experiment_dir, "checkpoints", "best_{}.pt".format("agent")
                    ),
                )
            # perform evaluation with render
    ...
    def record_transition(
        ...
        if finished_episodes.numel():
            ...
            self._track_rewards.extend(
                self._cumulative_rewards[finished_episodes][:, 0].reshape(-1).tolist()
            )
            self._track_timesteps.extend(
                self._cumulative_timesteps[finished_episodes][:, 0].reshape(-1).tolist()
            )
            # print(f"{finished_episodes.flatten()=}")
            # print(f"{terminated.flatten()=}")
            # print(f"{truncated.flatten()=}")
        ...
        if self.write_interval > 0:
            ...
            # print("\t\t mu track length is", len(self._track_rewards))
            if len(self._track_rewards):
                track_rewards = np.array(self._track_rewards)
                track_timesteps = np.array(self._track_timesteps)
                # print(f"{track_timesteps=}")
                # print(f"{np.max(track_timesteps)=}")
                # print(f"{np.min(track_timesteps)=}")
                # print(f"{np.mean(track_timesteps)=}")
        ...
        if type(modules) is dict:
            for name, data in modules.items():
                ...
                if module is not None:
                    ...
                    logger.warning(
                        "Cannot load the {} module. The agent doesn't have such an instance".format(
                            name
                        )
                    )
    ...
    def migrate(
        ...
        if "state_preprocessor" in self.checkpoint_modules:
            if "running_mean_std.running_mean" in checkpoint["model"]:
                state_dict = copy.deepcopy(
                    self.checkpoint_modules["state_preprocessor"].state_dict()
                )
                state_dict["running_mean"] = checkpoint["model"][
                    "running_mean_std.running_mean"
                ]
                state_dict["running_variance"] = checkpoint["model"][
                    "running_mean_std.running_var"
                ]
                state_dict["current_count"] = checkpoint["model"][
                    "running_mean_std.count"
                ]
                self.checkpoint_modules["state_preprocessor"].load_state_dict(
                    state_dict
                )
        ...
        if "value_preprocessor" in self.checkpoint_modules:
            if "value_mean_std.running_mean" in checkpoint["model"]:
                state_dict = copy.deepcopy(
                    self.checkpoint_modules["value_preprocessor"].state_dict()
                )
                state_dict["running_mean"] = checkpoint["model"][
                ...
                self.checkpoint_modules["value_preprocessor"].load_state_dict(
                    state_dict
                )
    ...
    def post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        if (
            ...
            # print(f"{self.tracking_data.keys()=}")
            # update best models
            reward = np.mean(
                self.tracking_data.get("Reward / Total reward (mean)", -(2**31))
            )
            # print("\t reward comp: ", reward, self.checkpoint_best_modules["reward"])
            if reward > self.checkpoint_best_modules["reward"]:
                # print("\t\t new best!: ", self.checkpoint_best_modules["reward"])
