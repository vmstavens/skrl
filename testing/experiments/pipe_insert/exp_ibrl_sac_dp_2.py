import os
import sys
from datetime import datetime
from pathlib import Path

import torch

from performance import save_timings, timer
from testing.experiments.pipe_insert.exp_utils import (
    exp_set_seed,
    get_dp_config,
    get_drlr_config,
    get_expert_memory_2,
    get_memory,
    get_td3_models,
    get_trainer,
    setup_environment,
)
from testing.shen.diffusion_policy_state import DiffusionPolicy
from testing.shen.dp_models import ConditionalUnet1D, EMAModel
from testing.shen.ibrl_sac_o_o2_clean_test import IBRL

# Source - https://stackoverflow.com/a/74483570
# Posted by Dev Gurung
# Retrieved 2026-02-09, License - CC BY-SA 4.0

os.environ["XLA_PYTHON_CLIENT_PREALLOCATE"] = "false"
os.environ["XLA_PYTHON_CLIENT_MEM_FRACTION"] = ".50"
os.environ["XLA_PYTHON_CLIENT_ALLOCATOR"] = "platform"


expert_data_dir = "testing/experiments/pipe_insert/run/data_collection_2_rot"
# expert_data_dir = "testing/experiments/pipe_insert/run/data_collection_2"

date_time = datetime.now()
time_stamp = date_time.strftime("%Y%m%d_%H_%M_%S")

actor = "il"
rollout_every_episodes = 10  # 100 manual rollout
# rollout_every_episodes = 20  # 100 manual rollout
run_rollout_only = False  # True
dpiter = 8  # 20
pred_horizon = 8
# dpiter = 5  # 20
# dpiter = 5  # 20
num_envs = int(os.environ.get("PIPE_INSERT_NUM_ENVS", "128"))
# num_envs = 10
training_timesteps = 400_000
# num_envs = 1
# num_envs = 900
# num_envs = 100
# num_envs = 100
# it takes about 2000 timesteps for dp to solve the problem
episode_length = 10_000  # 1000
# episode_length = 3800  # 1000
rl_il_equal_scale = False  # make sure that the rl action is on the same scale as
# rl_il_equal_scale = True  # make sure that the rl action is on the same scale as

wandb = True  # True
il_action_scale = 1.0
# il_action_scale = 0.15
# il_action_scale = 0.17
# warmup_timesteps = 10_000
warmup_timesteps = 10_000

dctrl_rl = [-0.005, 0.005]  # ±0.5cm

# il_action_scale = 0.17
reward_str = "dense"


exp_name = (
    Path(__file__).stem
    + f"_dpiter_{dpiter}"
    + f"_actor_{actor}"
    + f"_{reward_str}_"
    + time_stamp
)
exp_dir = Path(__file__).parent


def load_trained_dp_policy(
    checkpoint_path: Path, device: torch.device, config: dict, a_dim: int, o_dim: int
) -> DiffusionPolicy:
    """Load a trained diffusion policy for IL guidance."""

    dp_models: dict = {}
    dp_models["model"] = ConditionalUnet1D(a_dim=a_dim, o_dim=o_dim, config=config)
    ema = EMAModel(dp_models["model"].parameters(), power=config["ema_power"])
    dp_models["ema_model"] = ConditionalUnet1D(a_dim=a_dim, o_dim=o_dim, config=config)

    agent = DiffusionPolicy(
        a_dim=a_dim,
        o_dim=o_dim,
        models=dp_models,
        ema=ema,
        device=device,
        config=config,
    )
    agent = agent.load(checkpoint_path, device=device)

    return agent


# 0) set seed
exp_set_seed()

# 1) setup the environment
env = setup_environment(
    batch_size=num_envs,
    version=3,
    # version=2,
    episode_length=episode_length,
    auto_reset=True,
    # warp=True,
    impl="warp",
    skrl=False,
)

# 3) get bc config
drlr_config = get_drlr_config(exp_name, env, wandb=wandb)

drlr_config["actor"] = actor
drlr_config["il_ctrl_scale"] = il_action_scale
drlr_config["warmup_timesteps"] = warmup_timesteps
drlr_config["rl_il_equal_scale"] = rl_il_equal_scale
drlr_config["dctrl_rl"] = dctrl_rl

dp_config = get_dp_config(exp_name=exp_name, env=env)

# 2) setup drlr models
rl_models = get_td3_models(env)

a_dim = env.action_space.shape[0]
o_dim = env.observation_space.shape[0]

dp_checkpoint = "testing/experiments/pipe_insert/.runs/data_100_2_1_action_horizon_rot_20260215_160452/models/latest_model.pth"
# dp_checkpoint = "testing/experiments/pipe_insert/.runs/data_100_2_1_action_horizon/models/latest_model.pth"
# dp_checkpoint = "testing/experiments/pipe_insert/.runs/data_100_2_1_action_horizon/models/latest_model.pth"
# dp_checkpoint = "testing/experiments/pipe_insert/.runs/data_100_2_1_action_horizon/models/latest_model.pth"
dp_policy = load_trained_dp_policy(
    checkpoint_path=dp_checkpoint,
    device=env.device,
    config=dp_config,
    a_dim=a_dim,
    o_dim=o_dim,
)

# to decrease inference time, we reduce the number of diffusion iterations for diffusion policy,
# turns out 100 is a lot for our case, and 2 can just as easily be applied. We can also use 1 to make it bad
# dp_policy._num_diffusion_iters = 1
dp_policy._num_diffusion_iters = dpiter
dp_policy._pred_horizon = pred_horizon
# we also just need one sample from the prediction trajectory, and thus
dp_policy._act_horizon = 1


il_models = {"policy": dp_policy}


# 4) get expert data and memory
expert_memory = get_expert_memory_2(
    expert_data_dir=expert_data_dir,
    states_label="states_dense",
    actions_label="actions",
    rewards_label="rewards_dense",
    next_states_label="next_states_dense",
    dones_label="dones",
)

memory = get_memory(env, capacity=100_000)

# 5) get device
device = env.device

# 6) define agent
agent = IBRL(
    models=rl_models,
    models_il=il_models,
    memory=memory,
    expert_memory=expert_memory,
    cfg=drlr_config,
    observation_space=env.observation_space,
    action_space=env.action_space,
    device=device,
)


# Configure and instantiate the RL trainer
# rollout_video_dir = Path("/home/vims/git/skrl/testing/experiments/pipe_insert_3/media")
# rollout_video_dir = Path("testing/experiments/pipe_insert/media")
rollout_video_dir = exp_dir / f".runs/{exp_name}/media"
print("trainer...")
trainer = get_trainer(
    env,
    agent,
    timesteps=training_timesteps,
    # timesteps=1_000_000,
    trainer_cfg={
        "rollout_video_every_episodes": rollout_every_episodes,
        "rollout_video_num_steps": episode_length,
        "rollout_video_dir": rollout_video_dir.as_posix(),
        "rollout_video_prefix": "train_rollout",
        "rollout_video_env_index": 0,
        "log_rollout_path": "testing/experiments/pipe_insert/tmp/success_ibrl_mjx.json",
        "log_rollout_steps": episode_length,
        "log_rollout_exit": False,
    },
)
if run_rollout_only:
    print("recording rollout video for env 0...")
    trainer._record_rollout_video(0)
    print("rollout complete. exiting.")
    sys.exit(0)


print("training...")
with timer("train"):
    trainer.train()

save_timings(exp_dir / "results/models" / exp_name / "performance.json")
