"""Clean IBRL + DP training script using PipeInsert2 (pipe_insert_2_new)."""
...
@dataclass
class Args:
    """IBRL + Diffusion Policy on PipeInsert2 (pipe_insert_2_new)."""

    expert_data_dir: Path = Path(
        "testing/experiments/pipe_insert/run/data_collection_2_mocap"
    )
    ...
    #
    # OBS the actions limits for DP (KP = 1)
    # max delta pos = 0.00095 m
    # min delta pos = -0.0010357142857142859 m
    # max delta rot = 0.001742857142857143 rad
    # min delta rot = -0.0022714285714285714 rad
    #
    # if we round we get (again KP = 1)
    #
    # max delta pos = 0.001 m
    # min delta pos = -0.001 m
    # max delta rot = 0.002 rad
    # min delta rot = -0.002 rad
    #
    # because of this action space, we need to scale the episode length as well
    # performing a rollout of
    #
    action_trans_high: Optional[float] = 0.001
    action_trans_low: Optional[float] = -0.001
    action_rot_high: Optional[float] = 0.002
    action_rot_low: Optional[float] = -0.002
...
def main(args: Optional[Args] = None) -> None:
    ...
    eval_env = None
    if args.rollout_every_episodes > 0:
        if args.render_impl == "warp":
            raise ValueError(
                "Rollout videos are not supported with render_impl='warp'. "
                "Use --render-impl jax or --render-impl c, or set --rollout-every-episodes 0."
            )
        eval_env = setup_mocap_environment(
            batch_size=args.render_num_envs,
            episode_length=args.episode_length,
            auto_reset=True,
            action_repeat=1,
            impl=args.render_impl,
        )
        if args.action_dim_override is not None:
            override_action_space(
                env=eval_env,
                ...
                low=action_low,
                high=action_high,
                num_envs=args.render_num_envs,
            )

    exp_name = (
        Path(__file__).stem
        + f"_actor_{args.actor}_"
        + f"num_envs_{args.num_envs}_"
        + f"warmup_timesteps_{args.warmup_timesteps}_"
        + f"drlr_{args.decision_block}_"
        + datetime.now().strftime("%Y%m%d_%H_%M_%S")
    )

    # exp_name = (
    #     Path(__file__).stem
    #     + f"_dpiter_{args.dp_iters}"
    #     + f"_actor_{args.actor}"
    #     + "_dense_"
    #     + datetime.now().strftime("%Y%m%d_%H_%M_%S")
    # )
    exp_dir = Path(__file__).parent

    drlr_config = get_drlr_config(exp_name, env, wandb=args.wandb)
    ...
    print(
        "ibrl_sac_dp: training_timesteps="
        f"{args.training_timesteps} write_interval="
        f"{drlr_config['experiment']['write_interval']}"
    )
    ...
    if a_dim == 0:
        raise ValueError(
            "PipeInsert2 (pipe_insert_2_new) reports action_dim=0. "
    ...
    if args.dp_checkpoint is None:
        ...
        if default_ckpt.exists():
            dp_checkpoint = default_ckpt
        else:
            runs_dir = exp_dir / ".runs"
            dp_checkpoint = latest_checkpoint(runs_dir)
    else:
