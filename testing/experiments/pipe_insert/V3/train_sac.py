"""Pure SAC training script using the same V3 PipeInsert2 setup as DRLR."""
...
def validate_v3_env(env) -> None:
    ...
    if obs_dim != EXPECTED_OBS_DIM:
        raise ValueError(
            f"Expected V3 PipeInsert2 observation dim {EXPECTED_OBS_DIM}, got {obs_dim}"
        )
    if action_dim != EXPECTED_ACTION_DIM:
        raise ValueError(
            f"Expected V3 PipeInsert2 action dim {EXPECTED_ACTION_DIM}, got {action_dim}"
        )
...
@dataclass
class Args:
    """Pure SAC on V3 PipeInsert2 with DRLR-compatible CLI parameters."""
