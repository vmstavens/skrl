...
EXPECTED_RUN_TAG = "align_z_and_pos_x3_8"
# EXPECTED_RUN_TAG = "align_z_and_pos_x2_16"
# EXPECTED_RUN_TAG = "align_z_and_pos_x3_16"
# EXPECTED_RUN_TAG = "align_z_and_pos_x4_16"
...
class Args:
    ...
    record_path: Optional[Path] = Path(
        "testing/experiments/pipe_insert/V4/data/rollout_dp_reduced_se3_2.json"
    )
    ...
    def run(self) -> None:
        ...
        if not self.args.render:
            for _ in range(self.args.max_steps):
                ...
                prev_obs = self.obs_history[-1]
                self._apply_action(action)
                mj.mj_step(self.m, self.d)
                obs = self.get_obs()
                terminated = self._get_termination(obs)
                reward_metrics = self._compute_reward_metrics(prev_obs, obs, terminated)
                reward = reward_metrics["reward_total"]
                self._record_step(obs, action, reward, terminated, reward_metrics)
                self.obs_history.append(obs)
                self.episode_step += 1
                if terminated:
