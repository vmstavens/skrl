...
class PipeInsert2(mjx_env.MjxEnv):
    ...
    def __init__(
        ...
        self._reward_degree = config.reward_degree

        # self._termination_threshold_pos = 0.02 if not self._centi else 2
        # self._termination_threshold_rot = jp.deg2rad(15) if not self._degree else 15
        self._termination_threshold_pos = 0.01 if not self._centi else 1
        self._termination_threshold_rot = jp.deg2rad(10) if not self._degree else 10

        self._failed_pos_threshold = 0.5 if not self._centi else 50
    ...
    def _get_obs(self, data: mjx.Data) -> jax.Array:
        ...
        # [position error in keypoint frame, target z-axis in keypoint frame]
    ...
    #     # if self._reward_degree:
    #     #     rot_err = jp.degrees(rot_err)

    #     weighted_pos_err, weighted_rot_err = self._get_weighted_error_metrics(
    #         pos_err, rot_err
    #     )

    #     return -(weighted_pos_err + weighted_rot_err) / 300
    #     # return -(weighted_pos_err + weighted_rot_err)

    def _get_reward_vel_components(
        ...
        # Normalize by success thresholds so position (cm when centi=True) and
        # orientation (radians) are comparable. Orientation is weighted heavily
        # because z-axis alignment is the harder part of this task.
        eps = jp.asarray(1e-6)
        ...
        orientation_weight = jp.asarray(4.0)
        delta_pose_0 = jp.linalg.norm(jp.array([pos0, orientation_weight * rot0]))
        delta_pose_1 = jp.linalg.norm(jp.array([pos1, orientation_weight * rot1]))

        pose_progress = delta_pose_0 - delta_pose_1
        pos_progress = pos0 - pos1
        rot_progress = rot0 - rot1
        rot_distance_reward = -rot1

        near_pos = jp.exp(-pos1)
        near_rot = jp.exp(-rot1)
        near = 0.25 * near_pos + 1.75 * near_rot
        ...
        #     (
        #         "reward_components: \n"
        #         "\treward_weighted_position_error_prev={pos_err0}, \n"
        #         "\treward_weighted_rotation_error_prev={rot_err0}, \n"
        #         "\treward_weighted_position_error={pos_err1}, \n"
        #         "\treward_weighted_rotation_error={rot_err1}, \n"
        ...
        #     pos_err1=pos_err1,
        #     rot_err1=rot_err1,
        #     delta_pose_0=delta_pose_0,
        #     delta_pose_1=delta_pose_1,
        #     near=near,
        #     reward=reward,
        # )
        # jax.debug.callback(
        ...
        return {
            "reward_weighted_position_error_prev": w_pos_err0,
            "reward_weighted_rotation_error_prev": w_rot_err0,
            "reward_weighted_position_error": w_pos_err1,
            "reward_weighted_rotation_error": w_rot_err1,
            "reward_delta_pose_prev": delta_pose_0,
            "reward_delta_pose": delta_pose_1,
    ...
    #     self, data0: mjx.Data, data1: mjx.Data, done: jax.Array | None = None
    # ) -> dict[str, jax.Array]:
    #     _, pos_err0, rot_err0 = self._get_error_metrics(
    #         data0
    #     )  # R4 R(|delta pos|) R delta theta
    #     _, pos_err1, rot_err1 = self._get_error_metrics(
    #         data1
    #     )  # R4 R(|delta pos|) R delta theta
    ...
    #     # delta_s_max_pos = 1
    #     # delta_s_max_pos = 0.007
    #     # delta_s_max_rot = 1
    #     # delta_t_max = self._episode_length
    #     # delta_pose_0 = jp.linalg.norm(
    #     #     jp.array([pos_err0 / delta_s_max_pos, rot_err0 / delta_s_max_rot])
    #     # )
    #     # delta_pose_1 = jp.linalg.norm(
    #     #     jp.array([pos_err1 / delta_s_max_pos, rot_err1 / delta_s_max_rot])
    #     # )

    #     delta_pose_0 = jp.linalg.norm(jp.array([w_pos_err0, w_rot_err0]))
    #     delta_pose_1 = jp.linalg.norm(jp.array([w_pos_err1, w_rot_err1]))

    #     # pose_progress = delta_pose_0 - delta_pose_1
    #     # progress_reward = pose_progress
    #     # progress_reward = pose_progress / delta_t_max

    #     # reward = progress_reward * 5  # 3 wo. scaling

    #     # reward = 5.0 * (delta_pose_0 - delta_pose_1) - 0.01 * delta_pose_1
    #     near = 40 * jp.exp(-rot_err1)
    #     done_value = jp.array(0.0) if done is None else done
    #     reward = 5.0 * (delta_pose_0 - delta_pose_1) + near + done_value * 100

    #     # close addition
    #     # near_pos = jp.exp(-pos_err1)
    #     # near_rot = jp.exp(-rot_err1)
    #     # near_rotation_reward = near_rot * 10000
    #     # near_rotation_reward = near_rot * 0.8
    #     # close addition

    #     # scale = 500

    #     # reward = (progress_reward + near_rotation_reward) * scale
    ...
    def _get_empty_reward_metrics(self) -> dict[str, jax.Array]:
        zero = jp.array(0.0)
        return {
            "reward_weighted_position_error_prev": zero,
            "reward_weighted_rotation_error_prev": zero,
            "reward_weighted_position_error": zero,
            "reward_weighted_rotation_error": zero,
    ...
    def _get_reward_vel(self, data0: mjx.Data, data1: mjx.Data) -> float:
        return self._get_reward_vel_components(data0, data1)["reward_total"]

    def _get_reward_vel_reviced(self, data0: mjx.Data, data1: mjx.Data) -> float:
        _, pos_err0, rot_err0 = self._get_error_metrics(data0)
        _, pos_err1, rot_err1 = self._get_error_metrics(data1)
        ...
        pose0 = jp.linalg.norm(jp.array([pos0, orientation_weight * rot0]))
        pose1 = jp.linalg.norm(jp.array([pos1, orientation_weight * rot1]))

        pose_progress = pose0 - pose1
        pos_progress = pos0 - pos1
        rot_progress = rot0 - rot1

        pose_reward = 2.0 * pose_progress
        pos_reward = 0.5 * pos_progress
        rot_reward = 3.0 * rot_progress

        pose_penalty = -0.02 * pose1
        rot_penalty = -0.08 * rot1

        near_pos = jp.exp(-pos1)
        near_rot = jp.exp(-rot1)
        closeness_reward = 0.2 * near_pos + 0.8 * near_rot
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        # delta = jax.random.uniform(rng_delta, (3,), minval=0, maxval=0)
        delta = jax.random.uniform(rng_delta, (3,), minval=-0.3, maxval=0.3)
        try:
        ...
        obs, pos_err, rot_err = self._get_error_metrics(data)
        weighted_pos_err, weighted_rot_err = self._get_weighted_error_metrics(
            pos_err, rot_err
        )
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        if self._sparse_reward:
            ...
            # reward = self._get_pos_reward(data)
            reward_metrics = self._get_reward_vel_components(data0, data, done)
            reward = reward_metrics["reward_total"]
            # reward = self._get_reward_vel_reviced(data0, data)

            # reward = self._get_reward(data)
            # reward = -(self._w_pos * pos_err + self._w_rot * rot_err)
        ...
        # jax.debug.print(
        #     "p {pos_err:.4f}, r {rot_err:.4f}, done {done}",
        #     pos_err=pos_err,
        #     rot_err=rot_err,
        #     done=done,
        # )
