"""Clean IBRL + DP training script using V4 PipeInsert2 with domain randomization."""
...
def cable_randomization_stiffness_range(
    ...
    """Returns the same cable joint stiffness span used by V4/dr.ipynb."""
    ...
    # Match testing.experiments.pipe_insert.V4.env.domain_randomize and dr.ipynb.
...
def randomize_cable_length(
    ...
    """Randomize the V4 cable length by scaling each segment in MJX model fields."""
...
def latest_checkpoint(runs_dir: Path, run_tag: str | None = None) -> Path:
    pattern = (
        f"{run_tag}/models/latest_model.pth"
        if run_tag is not None
        else "align_z_and_pos_*_16/models/latest_model.pth"
    )
...
def main(args: Optional[Args] = None) -> None:
    ...
    if args.dp_checkpoint is None:
        dp_checkpoint = latest_checkpoint(exp_dir / ".runs", run_tag=args.dp_run_tag)
    else:
