...
from testing.experiments.pipe_insert.V3.train_drlr_both import setup_mocap_environment
from testing.experiments.pipe_insert.V3.utils import get_dp_config
...
def _latest_checkpoint(runs_dir: Path) -> Path:
    pattern = "data_mocap_x4_16/models/latest_model.pth"
...
class Args:
    ...
    record_path: Optional[Path] = Path(
        "testing/experiments/pipe_insert/V3/data/rollout_dp.json"
    )
