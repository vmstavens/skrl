...
from testing.experiments.pipe_insert.V4.train_sac import (
...
def _latest_sac_checkpoint(runs_dir: Path) -> Path:
    return Path(
        "testing/experiments/pipe_insert/V4/.runs/train_sac_actor_sac__warmup_timesteps_0__soft_update_beta_0.2_20260402_17_44_56/models/train_sac_actor_sac__warmup_timesteps_0__soft_update_beta_0.2_20260402_17_44_56/checkpoints/best_agent.pt"
    )
    patterns = [
...
class Args:
    ...
    record_path: Optional[Path] = Path(
        "testing/experiments/pipe_insert/V4/data/rollout_sac_test.json"
    )
...
class SACRollout:
    def __init__(
        self, env: PipeInsert2, model: mj.MjModel, data: mj.MjData, agent: SAC, args: Args
    ):
        self.env = env
        self.m = model
        self.d = data
    ...
    def get_obs(self) -> torch.Tensor:
        def _e_pos() -> np.ndarray:
            ...
            return t_w_target.R.T @ (t_w_keypoint.t - t_w_target.t)
...
def main(args: Optional[Args] = None) -> None:
    ...
    rollout = SACRollout(sim_env, model, data, agent, args)
    rollout.run()
