...
from testing.experiments.pipe_insert.V4.env import PipeInsert2
...
class Test:
    def __init__(self, env: PipeInsert2, model: mj.MjModel, data: mj.MjData):
        ...
        self.record_dir = Path(__file__).resolve().parent / "demos/reduced_se3"
    ...
    def get_obs(self) -> np.ndarray:
        ...
        dp_world = T_w_keypoint.t - T_w_target.t
        dp = T_w_target.R.T @ dp_world
        z_rel = T_w_target.R.T @ T_w_keypoint.R[:, 2]

        return np.concatenate([dp, z_rel], axis=0).astype(np.float64)

    def get_reward(self, obs: np.ndarray, terminated: bool) -> float:
        if self._sparse_reward:
            return float(terminated)
        pos_err, rot_err = self.get_error_terms(obs)
        return float(-(self._w_pos * pos_err + self._w_rot * rot_err))

    def get_error_terms(self, obs: np.ndarray) -> tuple[float, float]:
        pos_err = float(np.linalg.norm(obs[:3]))
        cos_z_alignment = np.clip(float(obs[5]), -1.0, 1.0)
        orient_err = float(np.arccos(cos_z_alignment))
    ...
    def get_termination(self, obs: np.ndarray) -> bool:
        ...
        print(f"pos {pos_err < pos_thresh} z_align {orient_err < rot_thresh}")
