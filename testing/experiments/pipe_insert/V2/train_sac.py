"""Pure SAC training script using the same V2 PipeInsert2 setup as DRLR."""
...
def get_comparable_sac_config(exp_name: str, env, wandb: bool, warmup_timesteps: int):
    ...
    # Closest SAC analogue to DRLR warmup: random exploration before updates start.
    cfg["random_timesteps"] = warmup_timesteps
    cfg["learning_starts"] = warmup_timesteps
...
@dataclass
class Args:
    """Pure SAC on V2 PipeInsert2 with DRLR-compatible CLI parameters."""
    ...
    action_trans_high: Optional[float] = 0.003
    action_trans_low: Optional[float] = -0.003
