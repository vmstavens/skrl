...
processed_data_dir = "data/mj_data/0_degrees_filt_sm"


date_time = datetime.now()
time_stamp = date_time.strftime("%Y%m%d_%H_%M_%S")

exp_name = Path(__file__).stem + "_block_" + time_stamp
...
# dp_checkpoint = exp_dir / ".runs" / "models" / "model_epoch_70.pth"
...
trainer = get_trainer(env, agent, timesteps=10_000_000)
with timer("train"):
    # start training
    trainer.train()
