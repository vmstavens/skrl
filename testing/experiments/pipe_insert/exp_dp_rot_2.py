"""Finetune diffusion policy on a new pipe_insert dataset (rotation)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Finetune DP policy (rotation) on a new dataset"
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        help="Path to the new dataset folder (json demos).",
        default="testing/experiments/pipe_insert/run/data_collection_2_rot_continued",
    )
    parser.add_argument(
        "--resume",
        type=Path,
        default="testing/experiments/pipe_insert/.runs/data_100_2_1_action_horizon_rot_20260214_192555/models/latest_model.pth",
        help="Path to a checkpoint (latest_model.pth). Defaults to latest run.",
    )
    ...
    resume_path = args.resume or _latest_checkpoint(runs_dir)
    ...
    # Use checkpoint config for horizons to keep dataset aligned.
    ...
    dp_config = get_dp_config(exp_name="pipe_insert_2_rot_finetune", env=env)
    ...
    run_dir = (
        ...
        / f".runs/finetune_{int(args.pct * 100)}_2_1_action_horizon_rot_{timestamp}"
    )
