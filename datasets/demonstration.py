...
#                     episodes.append(
#                         {"states": states, "actions": actions, "length": len(states)}
#                     )
...
#         """Normalize all data to [-1, 1] range."""
#         normalized_episodes = []
#         for episode in self.episodes:
...
#                 {"states": states, "actions": actions, "length": episode["length"]}
#             )
...
#         """Get a sample as (observations, actions) tuple."""
...
#         states_tensor = torch.from_numpy(states_seq).float()
#         actions_tensor = torch.from_numpy(actions_seq).float()

#         # Return observations (first obs_horizon steps) and all actions
#         return states_tensor[: self.obs_horizon], actions_tensor

#     def get_data_stats(self) -> Dict:
#         """Get dataset statistics."""
#         return self.stats if self.normalize else None
...
#         print(f"Batch - obs: {obs.shape}, actions: {actions.shape}")
#         break
...
class DemonstrationDataset(Dataset):
    def __init__(
        self,
        json_paths: List[Union[str, Path]],
        pred_horizon: int,  # Changed from sequence_length to match attractor dataset
        obs_horizon: int,
        action_horizon: int,
        state_key: str = "states",
        action_key: str = "actions",
        ...
        """
        A demonstration dataset that returns (obs_traj, action_traj) like AttractorTrajectoryDataset.
        
        Args:
            json_paths: List of paths to JSON files containing demonstrations
            pred_horizon: Length of each trajectory (matches AttractorTrajectoryDataset)
            obs_horizon: Number of observation steps to use
            action_horizon: Number of action steps to use
            state_key: Key for states in JSON files
            action_key: Key for actions in JSON files
        ...
        # Create sample indices that respect episode boundaries
        self.indices = self._create_sample_indices(
            episode_ends, pred_horizon, pad_before, pad_after
        )
    ...
    def _create_sample_indices(
        ...
        for episode_idx, episode in enumerate(self.episodes):
            ...
            # Calculate valid start positions within this episode
            for start_idx in range(
                pad_before, episode_length - pred_horizon + 1 - pad_after
            ):
                buffer_start = episode_start + max(0, start_idx - pad_before)
                buffer_end = episode_start + min(
                    episode_length, start_idx + pred_horizon + pad_after
                )

                sample_start = (
                    pad_before if start_idx >= pad_before else pad_before - start_idx
                )
                sample_end = sample_start + pred_horizon
    ...
    def __getitem__(self, idx: int):
        ...
        (pred_horizon, state_dim), matching AttractorTrajectoryDataset.
        """
        episode_idx, buffer_start, buffer_end, sample_start, sample_end = self.indices[
            idx
        ]
        ...
        # Extract the sequence for states and actions
        # These will be of length pred_horizon
        states_seq = episode["states"][sample_start:sample_end]  # (pred_horizon, state_dim)
        actions_seq = episode["actions"][sample_start:sample_end]  # (pred_horizon, action_dim)

        # Convert to torch tensors
        obs_traj = torch.from_numpy(states_seq).float()
        action_traj = torch.from_numpy(actions_seq).float()

        # Return in the same format as AttractorTrajectoryDataset
