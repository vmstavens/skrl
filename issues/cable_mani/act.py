...
def init() -> mj.MjModel:
    ...
    _f = scene.worldbody.add_frame(name="gripper", pos=[0, 0, 0.1])

    b_gripper_root = gripper.worldbody.first_body()

    b_gripper_root.add_site(name="root")

    x_lim, y_lim, z_lim = [-1, 1], [-1, 1], [-1, 1]
    roll_lim, pitch_lim, yaw_lim = [-1.57, 1.57], [-1.57, 1.57], [-1.57, 1.57]

    # b_gripper_root.add_joint(
    #     name="x", axis=[1, 0, 0], type=mj.mjtJoint.mjJNT_SLIDE, range=x_lim
    # )
    # b_gripper_root.add_joint(
    #     name="y", axis=[0, 1, 0], type=mj.mjtJoint.mjJNT_SLIDE, range=y_lim
    # )
    # b_gripper_root.add_joint(
    #     name="z", axis=[0, 0, 1], type=mj.mjtJoint.mjJNT_SLIDE, range=z_lim
    # )
    ...
    # scene.add_actuator(
    #     name="x", target="gripper/x", trntype=mj.mjtTrn.mjTRN_JOINT, ctrlrange=x_lim
    # ).set_to_position(kp=kp_pos, kv=kv_pos)

    # scene.add_actuator(
    #     name="y", target="gripper/y", trntype=mj.mjtTrn.mjTRN_JOINT, ctrlrange=y_lim
    # ).set_to_position(kp=kp_pos, kv=kv_pos)

    # scene.add_actuator(
    #     name="z", target="gripper/z", trntype=mj.mjtTrn.mjTRN_JOINT, ctrlrange=z_lim
    # ).set_to_position(kp=kp_pos, kv=kv_pos)
